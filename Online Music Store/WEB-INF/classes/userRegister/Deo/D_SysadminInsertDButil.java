...
public class D_SysadminInsertDButil {
		...
		try {
			...
			String sql = "insert into onlmusicmngsystem.event values(0,'"+ename+"','"+edate+"','"+ecode+"','"+ecgory+"','"+etheme+"','"+evenue+"','"+edescript+"','"+eorgname+"','"+eorgdt+"')";
		...
		public static List<D_Sysadmin> getupdatejv(){
			...
			try {	
				con = DBConnect.getConnection();
				stmt = con.createStatement();
				String sql ="select * from onlmusicmngsystem.event";
